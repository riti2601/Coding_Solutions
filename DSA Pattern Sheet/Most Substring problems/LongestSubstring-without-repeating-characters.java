...
public int lengthOfLongestSubstring2(String s) {
    ...
    while (end < s.length()) {
        ...
        if (map[c1] > 0) // If character is encountered again in current window, increment counter
            counter++;
        map[c1]++; // Increment character frequency in map
        end++; // Move end pointer forward

        // Shrink window if there are duplicate characters
        while (counter > 0) {
            final char c2 = s.charAt(start);
            if (map[c2] > 1) // If character frequency becomes greater than 1 after decrementing, decrement counter
                counter--;
